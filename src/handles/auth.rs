...
pub async fn login(
    ...
    // sql
    let user = sqlx::query_as::<_, User>(
        r##"SELECT id, name, email, github_id, avatar, role, plan, subscription_status, subscription_start_at, subscription_end_at, last_login_at, password_hash, created_at, updated_at FROM "user" WHERE name = $1"##,
    )
    .bind(username)
